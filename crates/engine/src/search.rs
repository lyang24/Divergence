...
async fn disk_graph_search_pipe_v3_inner(
    ...
    loop {
        ...
        // Check residency BEFORE get_or_load for free expansion (cache miss detection).
        let was_resident = if freeexp.is_some() { pool.is_resident(page_id) } else { true };
        ...
        if let Some((page_to_vids, max_bonus)) = freeexp {
            if !was_resident && perf.bonus_scored < max_bonus {
                if let Some(page_vids) = page_to_vids.get(page_id as usize) {
                    for &co_vid in page_vids {
                        ...
                        if perf.bonus_scored >= max_bonus { break; }
                    }
                }
            }
        }
